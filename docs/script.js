// ------------------- API helpers -------------------
const API = {
  listImages: () => fetch(u('/images')).then(r => r.json()),
  upload: (file) => {
    const fd = new FormData();
    fd.append('file', file);
    return fetch(u('/images'), { method: 'POST', body: fd })
      .then(r => { if (!r.ok) throw new Error('Upload failed'); return r.json(); });
  },
  uploadBatch: (files) => {
    const fd = new FormData();
    for (const f of files) fd.append('files', f, f.name);
    return fetch(u('/images/batch'), { method: 'POST', body: fd })
      .then(r => { if (!r.ok) throw new Error('Batch upload failed'); return r.json(); });
  },
  deleteImage: (image_id, withResults=true) =>
    fetch(u(`/images/${encodeURIComponent(image_id)}?delete_results=${withResults ? 'true' : 'false'}`), { method: 'DELETE' })
      .then(r => { if (!r.ok) throw new Error('delete failed'); return r.json(); }),

  deleteAllImages: (withResults=false) =>
    fetch(u(`/images?delete_results=${withResults ? 'true' : 'false'}`), { method: 'DELETE' })
      .then(r => { if (!r.ok) throw new Error('bulk delete failed'); return r.json(); }),

  startOCR: (image_id) => fetch(u(`/ocr/jobs?image_id=${encodeURIComponent(image_id)}`), { method: 'POST' }).then(r => r.json()),
  startOCRBatch: (ids) => fetch(u('/ocr/jobs/batch'), { method: 'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ image_ids: ids }) }).then(r => r.json()),
  jobStatus: (job_id) => fetch(u(`/ocr/jobs/${encodeURIComponent(job_id)}`)).then(r => r.json()),
  getResults: (image_id) => fetch(u(`/images/${encodeURIComponent(image_id)}/results`)).then(r => r.json()),
  cleanResults: (image_id) => fetch(u(`/images/${encodeURIComponent(image_id)}/clean`), { method: 'POST' }).then(r => r.json()),
  getImageURL: (image_id) => u(`/images/${encodeURIComponent(image_id)}/file`),
  exportCSV: (image_id) => window.location.assign(u(`/images/${encodeURIComponent(image_id)}/export.csv`)),
  exportAllCSVs: () => window.location.assign(u('/exports/results.zip')),
  patchItem: (image_id, index, payload) => fetch(u(`/images/${encodeURIComponent(image_id)}/results/${index}`), { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) }).then(r => r.json()),
  removeItem: (image_id, index) => fetch(u(`/images/${encodeURIComponent(image_id)}/results/${index}/remove`), { method: 'POST' }).then(r => r.json()),
};
...
if (apiDocs && API_BASE) apiDocs.href = u('/docs');
refreshImages();
